...
# from backend.estimator_2d_yang import Estimator_2d
...
class MultiEstimator ( object ):
    ...
    def _estimate3d(self, img_id, show=False, plt_id=0):
        ...
        affinity_mat, features = self.extractor.get_affinity ( data_batch, rerank=self.cfg.rerank )
        if self.cfg.rerank:
        ...
        bin_match = match_mat[:, torch.nonzero ( torch.sum ( match_mat, dim=0 ) > 1.9 ).squeeze ()] > 0.9  # 选择元素之和大于1.9的column
        ...
        matched_list = [np.array ( i ) for i in matched_list]   # shape = (实际人数, 检测到该人的相机数)
        if self.cfg.hybrid:
...
if __name__ == '__main__':
    ...
    for imgs in tqdm ( test_loader ):
        ...
        info_dict = est.predict ( imgs=this_imgs, camera_parameter=test_camera_parameter, show=False,
