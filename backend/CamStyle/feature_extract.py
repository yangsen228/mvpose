...
class FeatureExtractor ( object ):
    ...
    def get_affinity(self, query_batch, output_feature='pool5', rerank=False, use_gpu=True):
        ...
        # Adapt to use dataloader
        # query = list ( map ( lambda x: [i[0] for i in x], query ) )
        query_features = extract_features ( self.model, query_batch, 1, output_feature, use_gpu=use_gpu )  # keys = ['0_0', '0_1', '1_0', '1_1', ...]; feat_shape = [2048]
        if rerank:
        ...
        return affinity, torch.cat ( [query_features[f].unsqueeze ( 0 ) for f, _, _ in query], 0 ).cpu()
